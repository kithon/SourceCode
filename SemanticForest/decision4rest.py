...
class DecisionForest(object):
    ...
    def getTree(self, data=None, signal=None):
        return DecisionTree(data, self.picture, signal, self.sig_picture, self.radius, self.num_function, self.condition, self.np_rng, self.file_name)
    ...
    def fit(self, picture, sig_picture, d_limit=None, overlap=True):
        ...
        for t in xrange(self.num_tree):
            ...
            # -*- tree -*-
            tree_dic[t] = self.getTree(data, signal)
        ...
        while True:
            # -*- single core -*-
            print_time("debug: fit", self.file_name)
            for t in xrange(self.num_tree):
                tree = tree_dic[t]
                predict_dic[t] = tree.fit()
            
            # accuracy
            max_score = 0
            total_score = 0
            print_time("debug: accuracy", self.file_name)
            for i,p in enumerate(sig_picture):
                w,h = p.getSize()
                max_score += w * h
                signal = [(i,j,k) for j in xrange(w) for k in xrange(h)]
                for s in signal:
                    label_list = [predict_dic[t][s] for t in xrange(self.num_tree)]
                    label = collections.Counter(label_list).most_common()[0][0]
                    x,y = s[1],s[2]
                    if p.getSignal(x,y) == label:
                        total_score += 1
            score = 1.0 * total_score / max_score
...
class DecisionTree(object):
    ...
    def __init__(self, data, picture, signal, sig_picture, radius, num_function, condition, np_rng, file_name):
        self.data = data
        self.picture = picture
        self.signal = signal
        self.sig_picture = sig_picture
    ...
    def fit(self, d_limit=None, overlap=True):
        # ----- initialize -----
...
def etrims_tree_makeup(radius, size, d_limit, unshuffle, cram, four, num, parameter, t_args, file_name):
    ...
    t_index = literal_eval(linecache.getline(parameter, 4+size))
