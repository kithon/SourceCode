...
class ExtremeBinaryDecisionTree(DecisionTree):
    __slots__ = ['radius', 'num_function', 'condition',
                 'np_rng', 'd_limit', 'file_name',
                 'picture', 'node_length', 'parameter_list',
                 'elm_hidden', 'elm_coef', 'visualize']
    def __init__(self, elm_hidden=None, elm_coef=None,
        ...
        self.elm_coef = elm_coef
        self.visualize = visualize
        self.file_name = 'ebdt_' + SUFFIX

    def getNode(self, data=None, depth=None):
        return ExtremeBinaryNode(data, self.picture, depth, self.generate_threshold, self.d_limit, self.radius, self.condition)
...
##########################################################
##  Extreme Binary Node (for etrims)
##########################################################

class ExtremeBinaryNode(Node):
    __slots__ = ['data', 'picture', 'depth', 'gen_threshold',
                 'd_limit', 'radius', 'condition', 'l_index', 'r_index',
                 'terminal', 'label', 'selected_dim', 'theta',
                 'betas', 'biases']
...
def etrims_tree(n_hidden = [1000], coef = [1000.], size=6, d_limit=None):
    ...
    # ExtremeBinaryDecisionTree
    elm_hidden = [3 * (2*size+1)*(2*size+1) * 2] # (3 channel) * (width) * (height) * 2

    print_time('train_ExtremeBinaryDecisionTree elm_hidden is %d, num function is %d' % (elm_hidden[0], num_function))
    ebdt = ExtremeBinaryDecisionTree(radius=size, elm_hidden=elm_hidden, elm_coef=None, num_function=num_function)
    ebdt.fit(train_set, d_limit=d_limit)

    print_time('test_ExtremeBinaryDecisionTree')
    score = ebdt.score(test_set)
    print_time('score is %f' % score)

    print_time('ExtremeBinaryDecisionTree info')
    ebdt.info()
