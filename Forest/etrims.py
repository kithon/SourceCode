...
class DecisionTree(object):
    ...
    def fit(self, picture, d_limit=None, overlap=True):
        ...
        for i,p in enumerate(picture):
            w,h = p.getSize()
            if overlap:
                input = {(i,j,k):index for j in xrange(w) for k in xrange(h)}
            else:
                input = {(i,j,k):index for j in xrange(self.radius, w, 2*self.radius+1) for k in xrange(self.radius, h, 2*self.radius+1)}                        
