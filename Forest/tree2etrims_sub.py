    ...
    for i in xrange(DATA_SIZE):
        ...
        for x,row in enumerate(signal_w_h):    
            for y,element in enumerate(row):
                ...
                temp = []
                print data_w_h
                print crop, len(crop), len(crop[0])
                for i in xrange(len(crop)):
                    temp.append(crop[i][y1:y2].tolist())
                crop = temp
                print x1,x2,y1,y2, len(data_w_h)
                print "crop", crop, len(crop)
                data = zeros(2*size+1, 2*size+1)
                data_x1, data_x2 = x1-(x-size), 2*size-((x+size)-x2)
                data_y1, data_y2 = y1-(y-size), 2*size-((y+size)-y2)
                ...
                for i in xrange(data_x1, data_x2):
                    for j in xrange(data_y1, data_y2):
                        ...
                        """
                        for k in xrange(3):
                            data[i][j][k] = crop[i-data_x1][j-data_y1][k]
                        """
                        #break
                    #break
                    #print crop[i-data_x1]
                    #data[i] = crop[i-data_x1]
                    #print data[i]
                    #print crop[i-data_x1]
